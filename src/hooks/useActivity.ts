...
export const useActivity = () => {
  ...
  const [viewingItem, setViewingItem] = useState<Activity | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await activityService.index({
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      const payload = {
        ...formData,
        implementationDate: formData.implementationDate ? formData.implementationDate.toString() : null,
      }
      if (editingItem) {
        await activityService.update(Number(editingItem.id), payload);
      } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await activityService.delete(deletingItem.id);
