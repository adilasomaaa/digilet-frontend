...
export const useLetter = (id?: string) => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    const data = {
      ...formData,
      letterHeadId: (formData.letterHeadId === "null" || formData.letterHeadId === "") ? null : formData.letterHeadId
    };
    try {
      if (editingItem) {
        await letterService.update(Number(editingItem.id), data);
      } else {
        await letterService.create(data);
      }
