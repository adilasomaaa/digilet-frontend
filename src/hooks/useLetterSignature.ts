...
export const useLetterSignature = (studentLetterSubmissionId?: string, generalLetterSubmissionId?: string) => {
  ...
  const [editingItem, setEditingItem] =
    useState<LetterSignature | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await letterSignatureService.index({
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await letterSignatureService.create(payload);
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await letterSignatureService.delete(deletingItem.id);
