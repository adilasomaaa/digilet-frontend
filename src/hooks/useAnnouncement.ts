...
export const useAnnouncement = (institutionId?: number) => {
  ...
  const fetchItems = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await announcementService.index({
  ...
  const onSubmit = async (formData: any) => {
    ...
    try {
      if (editingItem) {
        await announcementService.update(Number(editingItem.id), formData);
      } else {
